...
import frc.robot.util.LimelightFrontMiddle;
import frc.robot.util.Constants.HubAlignConstants;
...
public class AlignToHubBasisVectorWithTranslation extends Command {
    ...
    private double lateralLowerPThreshold;
    private Limelight limelightFrontMiddle;
    ...
    public AlignToHubBasisVectorWithTranslation() {
        ...
        rotationalLowerPThreshold = HubAlignConstants.kRotationalLowerPThreshold;
    ...
    public void execute() {
        ...
        // Optional<Pose2d> estimatedPoseOptional = limelightFrontMiddle.getEstimatedPoseMT2();
        // Pose2d estimatedPose = estimatedPoseOptional.get();

        // lateralError = estimatedPose.getX() - desiredX;
        lateralError = limelightFrontMiddle.getTx() - desiredX;
        if (Math.abs(lateralError) > lateralLowerPThreshold)
            rotationalPIDController.setP(lateralP);
        else
            rotationalPIDController.setP(lateralLowerP);
        rotationalPIDController.setI(lateralI);
        rotationalPIDController.setD(lateralD);
